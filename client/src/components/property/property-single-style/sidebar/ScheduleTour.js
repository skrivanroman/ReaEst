	...
	return (
		<div className="ps-navtab">
			<ul className="nav nav-pills mb-3" id="pills-tab" role="tablist">
				{tabs.map((tab) => (
					<li className="nav-item" key={tab.id} role="presentation">
						<button
							className={`nav-link${tab.id === 'inperson' ? ' active mr15 mb5-lg' : ''}`}
							id={`pills-${tab.id}-tab`}
							data-bs-toggle="pill"
			...
			<div className="tab-content" id="pills-tabContent">
				{tabs.map((tab) => (
					<div
						className={`tab-pane fade${tab.id === 'inperson' ? ' show active' : ''}`}
						id={`pills-${tab.id}`}
						role="tabpanel"
						...
						<form className="form-style1">
							<div className="row">
								<div className="col-md-12">
									<div className="mb20">
										<input type="text" className="form-control" placeholder="Čas" required />
									</div>
								</div>
								{/* End .col-12 */}
								...
								<div className="col-md-12">
									<div className="mb10">
										<textarea cols={30} rows={4} placeholder="Zpráva pro majitele" defaultValue={''} />
									</div>
								</div>
								{/* End .col-12 */}
