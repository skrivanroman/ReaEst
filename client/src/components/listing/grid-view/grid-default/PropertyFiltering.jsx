	...
	useEffect(() => {
		...
		if (priceRange.length > 0) {
			const filtered = refItems.filter(
				(elm) =>
					Number(elm.price.split('K')[0].split(' ').join('')) >= priceRange[0] &&
					Number(elm.price.split('K')[0].split(' ').join('')) <= priceRange[1]
			)
			filteredArrays = [...filteredArrays, filtered]
		}
		...
		if (squirefeet.length > 0 && squirefeet[1]) {
			...
			const filtered = refItems.filter((elm) => elm.sqft >= Number(squirefeet[0]) && elm.sqft <= Number(squirefeet[1]))
			filteredArrays = [...filteredArrays, filtered]
		}
		if (yearBuild.length > 0) {
			const filtered = refItems.filter(
				(elm) => elm.yearBuilding >= Number(yearBuild[0]) && elm.yearBuilding <= Number(yearBuild[1])
			)
			filteredArrays = [...filteredArrays, filtered]
		}
